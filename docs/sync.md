...
| **Rebuild OCR documents** | The *Documents* button on the Sync screen, with a confirmation. Nothing is deleted on the server: the phone picks out of its own copy of the index the photos that look like documents (`PhotoCache.docsLikeDocuments`, the server's own rule — receipt, label, screenshot, and anything text was already read out of), queues them and starts a sync, and each new document takes the old one's place, with the reading cleaned on the way out of the server's cache. Free — a photo read once is never read again |
