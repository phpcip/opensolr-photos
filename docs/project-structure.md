...
| Change what the phone's copy of the index holds | `EditRepository.CLONE_FIELDS`, `PhotoCache.Doc` and the `docs` table, and the clone format version in `AppPrefs` — raising it forces the copy to be read from the index again. A new column is a new `PhotoCache.VERSION` step in `onUpgrade` that fills it from the stored documents with `forEachPage` (never all at once, never inside a transaction of its own). |
