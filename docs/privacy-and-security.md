...
| Location (`ACCESS_COARSE_LOCATION`, `ACCESS_FINE_LOCATION`, `ACCESS_BACKGROUND_LOCATION`) | Asked only if you turn on *Locate new photos*, so a new photo without a position takes the phone's, also in background syncs | No |
